...
| ---------------------------------- | ------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Compass                            | `0.55.4`                                          | Explicit migration required                                    | Convert resource bundles to plugins; replace ambient schema/database access; migrate WhatsApp and application processors; mount v1 or event-native Fetch handler; run tenant, account, sandbox, channel, chat, and admin suites. |
...
| Gilpinna                           | `0.48.0`                                          | Explicit migration required                                    | Port S3/canonical assets, collections, features, provider streaming, and server assembly; run its integration suite plus asset-backed image/tool coverage.                                                                       |
...
| `@copilotz/chat-adapter` / chat UI | HTTP/SSE consumer rather than core import         | Transitional v1 projection exists; package integration pending | Run its complete suite against `createV1FetchHandler()`, then migrate deliberately to attachment/event-native output without removing v1 first.                                                                                  |
...
| `@copilotz/sandbox`                | Oxian integration; no direct v3 core import found | No core source migration                                       | Keep its Oxian and persistence suites as an ecosystem gate; do not couple it to Copilotz internals.                                                                                                                              |

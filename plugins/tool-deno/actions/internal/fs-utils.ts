/**
 * Shares filesystem, snapshot, and diff utilities among Deno Tool Actions.
 *
 * @module
 */
...
export async function readWorkspaceFile(
  ...
  const safeEnd = Math.max(
    safeStart,
    Math.min(endLine, totalLines || safeStart),
  );
  const sliced = totalLines === 0 ? [] : lines.slice(safeStart - 1, safeEnd);
...
export async function listWorkspaceDirectory(
  ...
  const visit = async (absolutePath: string, depth: number): Promise<void> => {
    for await (const entry of denoNs.readDir!(absolutePath)) {
      ...
      if (
        entry.isDirectory &&
        isExcludedDir(entry.name, extraExcludes, includeAll)
      ) continue;
    ...
    for await (const entry of denoNs.readDir!(absolutePath)) {
      ...
      for (let index = 0; index < lines.length; index++) {
        ...
        while ((m = regex.exec(line)) !== null) {
          const entry: SearchCodeResult["matches"][number] = {
            line: index + 1,
            text: line,
          };
...
export async function applyWorkspacePatch(
  ...
  for (const operation of operations) {
    switch (operation.type) {
      ...
      case "insert_before": {
        const index = ensureAnchorOnce(
          content,
          operation.anchor,
          "insert_before",
        );
        content = `${content.slice(0, index)}${operation.content}${
          content.slice(index)
        }`;
        break;
      }
      case "insert_after": {
        ...
        content = `${content.slice(0, insertAt)}${operation.content}${
...
export async function getWorkspaceFileDiff(
  ...
  const current = await readWorkspaceFile(inputPath);
  const { hunks, truncated } = buildDiffHunks(
    snapshot.content,
    current.content,
  );

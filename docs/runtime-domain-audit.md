...
| Thread-specific operation catalog methods                                 | [Core operation queries](../plugins/core-http/adapters/http/core/operations/index.ts), consumed by the server after thread authorization. The catalog exposes generic SQL/session and operation operations. |

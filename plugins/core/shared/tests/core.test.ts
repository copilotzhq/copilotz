...
async function runCoreSuite(url: string, schema: string): Promise<void> {
  ...
  const { runtime, participants, threads, messages, store, session } = fixture;
  ...
  try {
    ...
    assertEquals(coreEvent(original.event).threadId, undefined);
    assertEquals(coreEvent(original.event).routing, {});
    assertEquals(coreEvent(original.event).visibility, { kind: "public" });
    assertEquals(
      (await session.query<{
        source_type: string | null;
        source_id: string | null;
    ...
    const active = projectActiveMessageBranch(history, {
      rootMessageId: "message-1",
      headMessageId: "message-1-rev",
      previousRevisionMessageId: "message-1",
      revisionIndex: 1,

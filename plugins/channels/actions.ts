...
const EGRESS_KEYS = new Set(["messageId"]);
...
function contentSequence(value: unknown): ContentSequence {
  const refs = dataArray(value, "Channel delivery content");
  return Object.freeze(refs.map((value, index) => {
    const cloned = cloneChannelJson(
      value,
      `Channel delivery content[${index}]`,
    );
    if (!cloned || typeof cloned !== "object" || Array.isArray(cloned)) {
      throw new TypeError(`Channel delivery content[${index}] must be a ref.`);
    }
    const ref = cloned as unknown as ContentRef;
    requiredText(ref.assetId, `Channel delivery content[${index}] Asset ID`);
    requiredText(ref.kind, `Channel delivery content[${index}] kind`);
    requiredText(ref.role, `Channel delivery content[${index}] role`);
    requiredText(
      ref.mediaType,
...
async function executeChannelEgress(
  ...
  const snapshot = dataObject(
    rawInput,
    EGRESS_KEYS,
    "Channel egress Action input",
  );
  const messageId = requiredText(
    snapshot.messageId,
    "Channel egress message ID",
  );
  ...
  const sender = await context.collections.participant.get({
    id: requiredText(message.senderId, "Message sender ID"),
  });
  if (!sender || sender.participantType !== "agent") {
    return Object.freeze({ intents: Object.freeze([]) });
  }
  const threadId = requiredText(message.threadId, "Message thread ID");
  ...
  for (const value of bindings) {
    ...
    const channelId = requiredText(binding.channelId, "Binding Channel ID");
    const resource = channelResource(context, channelId);
    if (resource.egress !== "external") continue;
    ...
    intents.push(Object.freeze({
      ...
      threadId,
      messageId,
      route: jsonObject(binding.route, "Channel delivery route"),

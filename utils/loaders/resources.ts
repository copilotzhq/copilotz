...
const loadResources = async (
  { path }: { path: string } = { path: "resources" },
): Promise<Resources> => {
  ...
  for await (const entry of readDir(agentsPath.replace("file://", ""))) {
    ...
    const config =
      await loadModuleSafe(agentsPath + entry.name + "/config.ts") ?? {};
  ...
  logPhase("agents", startedAt, { count: agentConfigs.length });
  ...
  for await (const entry of readDir(toolsPath.replace("file://", ""))) {
    ...
    const execute = await loadModuleSafe(
      toolsPath + entry.name + "/execute.ts",
    );
  ...
  try {
    for await (const evtDir of readDir(processorsPath.replace("file://", ""))) {
      ...
      const dirPath = (processorsPath + eventTypeDir + "/").replace(
        "file://",
        "",
      );
      for await (const file of readDir(dirPath)) {
        ...
        if (
          ...
          discovered.push({
            shouldProcess: maybeShouldProcess as (
              event: unknown,
              deps?: unknown,
            ) => boolean | Promise<boolean>,
            process: maybeProcess as (
              event: unknown,
              deps?: unknown,
  ...
  logPhase("skills", startedAt, { count: skills.length });

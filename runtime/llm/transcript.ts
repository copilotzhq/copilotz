...
/** Pure Web-compatible SHA-256 avoids runtime-specific crypto worker behavior. */
...
export async function prepareAttemptTranscript(args: {
  ...
  const promptFingerprint = fingerprintMessages(messages);
  const promptPrefixFingerprint = promptPrefixMessageCount === messages.length
    ? promptFingerprint

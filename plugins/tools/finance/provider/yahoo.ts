import type {
  ...
  ScreenSecuritiesOutput,
  SearchAssetsInput,
  SearchAssetsOutput,
} from "./types.ts";
import {
  acquireCookieAndCrumb,
  doAuthenticatedYahooRequest,
  doYahooRequest,
} from "../client/yahoo-client.ts";
...
/** Creates the stateless Yahoo Finance provider as a plain capability record. */
export function createYahooProvider(): FinanceDataProvider {
  return {
    async searchAssets(
      ...
      }&quotesCount=${limit}&newsCount=0&listsCount=0&quotesQueryId=tss_match_phrase_query`;
      ...
      if (input.quote_type) {
        results = results.filter((r: any) =>
          r.quote_type?.toLowerCase() === input.quote_type?.toLowerCase()
        );
      }
      if (input.exchange) {
        results = results.filter((r: any) =>
          r.exchange?.toLowerCase() === input.exchange?.toLowerCase()
        );
      }
      if (input.sector) {
        results = results.filter((r: any) =>
          r.sector?.toLowerCase() === input.sector?.toLowerCase()
        );
      }
      if (input.industry) {
        results = results.filter((r: any) =>
          r.industry?.toLowerCase() === input.industry?.toLowerCase()
        );
      }
    ...
    async getHistoricalPrices(
      ...
      if (frequency !== "daily" && frequency !== "monthly") {
        throw new FinanceError({
          code: "bad_request",
          message:
            `Unsupported frequency: ${frequency}. Only 'daily' and 'monthly' are supported.`,
        });
      }
      ...
      const p1 = input.start_date
        ? Math.floor(new Date(input.start_date).getTime() / 1000)
        : oneYearAgoSec;
      const p2 = input.end_date
        ? Math.floor(new Date(input.end_date).getTime() / 1000)
        : nowSec;

      const events = input.include_corporate_actions !== false
        ? "div|split"
        : "";
      const url = `https://query1.finance.yahoo.com/v8/finance/chart/${
        encodeURIComponent(input.symbol)
      }?period1=${p1}&period2=${p2}&interval=${interval}&events=${events}`;
      ...
      return {
        ...
        start_date: input.start_date ||
          new Date(oneYearAgoSec * 1000).toISOString().split("T")[0],
        end_date: input.end_date ||
          new Date(nowSec * 1000).toISOString().split("T")[0],
        bars,
        corporate_actions: corporateActions.length > 0
          ? corporateActions
          : undefined,
        truncated,
    ...
    async getAnalystRatings(
      ...
      const recentChanges = ugh
        ...
        .map((h: any) => ({
          ...
          action:
            h.action === "up" || h.action === "down" || h.action === "main" ||
              h.action === "init" || h.action === "reiter"
              ? h.action
              : "main",
    ...
    async getCalendarEvents(
      ...
      if (include.includes("earnings")) {
        const data = await fetchQuoteSummary<any>(input.symbol, [
          "calendarEvents",
        ], signal);
        const result = data?.quoteSummary?.result?.[0];
      ...
      if (include.includes("dividends")) {
        ...
        const url = `https://query1.finance.yahoo.com/v8/finance/chart/${
          encodeURIComponent(input.symbol)
        }?period1=${oneYearAgoSec}&period2=${nowSec}&interval=1mo&events=div`;
        const data = await doYahooRequest<any>(url, undefined, { signal });
        const result = data?.chart?.result?.[0];
        if (result?.events?.dividends) {
          for (const [ts, div] of Object.entries(result.events.dividends)) {
    ...
    async getOwnership(
      ...
      const modules: string[] = [];
      if (include.includes("institutions")) {
        modules.push("institutionOwnership");
      }
      if (include.includes("funds")) modules.push("fundOwnership");
      if (include.includes("insiders")) modules.push("insiderHolders");
      if (include.includes("major_holders")) {
        modules.push("majorHoldersBreakdown");
      }
      if (include.includes("insider_transactions")) {
        modules.push("insiderTransactions");
      }
      ...
      if (
        include.includes("insider_transactions") && result.insiderTransactions
      ) {
    ...
    async getFinancialStatements(
      ...
      const prefix = period_type === "annual"
        ? "annual"
      ...
      for (let i = 0; i < reqTypes.length; i += chunkSize) {
        ...
        const qs = `symbol=${encodeURIComponent(symbol)}&type=${
          encodeURIComponent(chunk.join(","))
        }&period1=${p1}&period2=${p2}&crumb=${encodeURIComponent(crumb)}`;
        const url =
      ...
      for (const item of allResults) {
        ...
        const normalizedMetric = typeKey.startsWith(prefix)
          ? typeKey.slice(prefix.length)
          : typeKey;

        // Map back to our keys
        const originalKey = keys.find((k) =>
          k.toLowerCase() === normalizedMetric.toLowerCase()
        );
        ...
        if (Array.isArray(series)) {
          ...
          for (const dataPoint of series) {
            ...
            const snakeMetric = originalKey.replace(/([A-Z])/g, "_$1")
      ...
      let periods = Object.values(periodsMap).sort((a, b) =>
    ...
    async screenSecurities(
      input: ScreenSecuritiesInput,
      signal?: AbortSignal,
    ): Promise<ScreenSecuritiesOutput> {
      ...
      if (size < 1 || size > maxSize) {
        throw new FinanceError({
          code: "bad_request",
          message:
            `Parameter 'size' must be between 1 and ${maxSize}. Got ${size}`,
        });
      }
      if (offset < 0) {
        throw new FinanceError({
      ...
      const validateRange = (field: string, range?: [number, number]) => {
        ...
        if (!Array.isArray(range)) {
          throw new FinanceError({
            code: "bad_request",
            message:
              `Invalid range format for '${field}': expected [min, max] array.`,
          });
        }
        const [min, max] = range;
        if (min > max) {
          throw new FinanceError({
            code: "bad_request",
            message:
              `Invalid range for '${field}': min (${min}) cannot be greater than max (${max})`,
          });
        }
      };
      ...
      if (quoteType === "INDEX") {
        ...
        validateRange(
          ...
          idxInput.fiftyTwoWeekPercentChangeRange,
        );
        ...
        validateRange(
          ...
          idxInput.intradayPriceChangeRange,
        );
        ...
        validateRange(
          "fiftyTwoWeekPercentChangeRange",
          eqInput.fiftyTwoWeekPercentChangeRange,
        );
        ...
        validateRange(
          ...
          eqInput.lastClosePriceBookValueRange,
        );
        // v2.4 Valuation range validations:
        validateRange(
          "lastCloseMarketCapRange",
          eqInput.lastCloseMarketCapRange,
        );
        validateRange(
          "lastCloseTevEbitdaRange",
          eqInput.lastCloseTevEbitdaRange,
        );
        validateRange(
          "lastClosePriceTangibleBookValueRange",
          eqInput.lastClosePriceTangibleBookValueRange,
        );
        ...
        validateRange(
          ...
          eqInput.lastCloseTevTotalRevenueRange,
        );
        validateRange(
          "lastClosePriceEarningsRatioRange",
          eqInput.lastClosePriceEarningsRatioRange,
        );
        validateRange(
          "priceBookRatioQuarterlyRange",
          eqInput.priceBookRatioQuarterlyRange,
        );
        // v2.4 ESG & Ownership range validations:
        validateRange(
          "environmentalScoreRange",
          eqInput.environmentalScoreRange,
        );
        ...
        validateRange(
          ...
          eqInput.insiderOwnershipPercentRange,
        );
        validateRange(
          "institutionalOwnershipPercentRange",
          eqInput.institutionalOwnershipPercentRange,
        );
        ...
        validateRange(
          ...
          eqInput.longTermDebtEquityRange,
        );
        validateRange(
          "returnOnTotalCapitalRange",
          eqInput.returnOnTotalCapitalRange,
        );
        ...
        validateRange(
          ...
          eqInput.forwardDividendYieldRange,
        );
        validateRange(
          "operatingCashFlowToCurrentLiabilitiesRange",
          eqInput.operatingCashFlowToCurrentLiabilitiesRange,
        );
        validateRange(
          "ebitdaInterestExpenseRange",
          eqInput.ebitdaInterestExpenseRange,
        );
        validateRange(
          "ebitInterestExpenseRange",
          eqInput.ebitInterestExpenseRange,
        );
        validateRange(
          "totalRevenue1YrGrowthRange",
          eqInput.totalRevenue1YrGrowthRange,
        );
        validateRange(
          "netIncome1YrGrowthRange",
          eqInput.netIncome1YrGrowthRange,
        );
        validateRange(
          "basicEpsContinuingOperationsRange",
          eqInput.basicEpsContinuingOperationsRange,
        );
        validateRange(
          "revenueGrowthPercentYoYQuarterlyRange",
          eqInput.revenueGrowthPercentYoYQuarterlyRange,
        );
        validateRange("totalRevenueRange", eqInput.totalRevenueRange);
        validateRange(
          "totalRevenueAnnualMarketCurrencyRange",
          eqInput.totalRevenueAnnualMarketCurrencyRange,
        );
        validateRange(
          "totalRevenuePerEmployeeAnnualMarketCurrencyRange",
          eqInput.totalRevenuePerEmployeeAnnualMarketCurrencyRange,
        );
        validateRange("netEpsBasicRange", eqInput.netEpsBasicRange);
        validateRange("ebitda1YrGrowthRange", eqInput.ebitda1YrGrowthRange);
        validateRange(
          "dilutedEps1YrGrowthRange",
          eqInput.dilutedEps1YrGrowthRange,
        );
        validateRange("netEpsDilutedRange", eqInput.netEpsDilutedRange);
        validateRange("netIncomeIsRange", eqInput.netIncomeIsRange);
        validateRange(
          "netIncomeIsAnnualMarketCurrencyRange",
          eqInput.netIncomeIsAnnualMarketCurrencyRange,
        );
        validateRange(
          "netIncomePerEmployeeAnnualMarketCurrencyRange",
          eqInput.netIncomePerEmployeeAnnualMarketCurrencyRange,
        );
        ...
        validateRange(
          "dilutedEpsContinuingOperationsRange",
          eqInput.dilutedEpsContinuingOperationsRange,
        );
        validateRange("ebitRange", eqInput.ebitRange);
        validateRange("bookValueShareRange", eqInput.bookValueShareRange);
        validateRange(
          "totalSharesOutstandingRange",
          eqInput.totalSharesOutstandingRange,
        );
        validateRange("totalDebtRange", eqInput.totalDebtRange);
        validateRange("totalAssetsRange", eqInput.totalAssetsRange);
        validateRange(
          "totalCashAndShortTermInvestmentsRange",
          eqInput.totalCashAndShortTermInvestmentsRange,
        );
        validateRange(
          "totalCurrentAssetsRange",
          eqInput.totalCurrentAssetsRange,
        );
        validateRange("totalEquityRange", eqInput.totalEquityRange);
        validateRange(
          "totalCommonSharesOutstandingRange",
          eqInput.totalCommonSharesOutstandingRange,
        );
        validateRange(
          "totalCurrentLiabilitiesRange",
          eqInput.totalCurrentLiabilitiesRange,
        );
        ...
        validateRange(
          "cashOnHandQuarterlyMarketCurrencyRange",
          eqInput.cashOnHandQuarterlyMarketCurrencyRange,
        );
        validateRange(
          "cashFromOperations1YrGrowthPercentRange",
          eqInput.cashFromOperations1YrGrowthPercentRange,
        );
        validateRange(
          "unleveredFreeCashFlowRange",
          eqInput.unleveredFreeCashFlowRange,
        );
        validateRange(
          "leveredFreeCashFlowRange",
          eqInput.leveredFreeCashFlowRange,
        );
        validateRange(
          "leveredFreeCashFlow1YrGrowthPercentRange",
          eqInput.leveredFreeCashFlow1YrGrowthPercentRange,
        );
        validateRange("dividendPerShareRange", eqInput.dividendPerShareRange);
        validateRange(
          "capitalExpenditureRange",
          eqInput.capitalExpenditureRange,
        );
        validateRange(
          "forwardDividendPerShareRange",
          eqInput.forwardDividendPerShareRange,
        );
        validateRange(
          "cashFromOperationsRange",
          eqInput.cashFromOperationsRange,
        );
      }

      // 2. Sort field mapping
      const sortField = input.sortField ??
        (quoteType === "INDEX" ? "regularMarketChangePercent" : "marketCap");
      ...
      const EQUITY_FIELD_MAP: Record<string, string> = {
        ...
        lastClosePriceTangibleBookValueLtm:
        ...
        lastCloseTevTotalRevenueLtm:
          "lastclosetevtotalrevenue.lasttwelvemonths",
        lastClosePriceEarningsRatioLtm:
        ...
        operatingCashFlowToCurrentLiabilitiesLtm:
        ...
        totalRevenueAnnualMarketCurrency:
          "total_revenue_market_currency.annual",
        totalRevenuePerEmployeeAnnualMarketCurrency:
        ...
        netIncomePerEmployeeAnnualMarketCurrency:
        ...
        dilutedEpsContinuingOperationsLtm:
        ...
        totalCashAndShortTermInvestmentsLtm:
        ...
        totalCommonSharesOutstandingLtm:
        ...
        cashOnHandQuarterlyMarketCurrency:
          "cash_on_hand_quarterly_market_currency",
        cashFromOperations1YrGrowthPercentLtm:
        ...
        leveredFreeCashFlow1YrGrowthPercentLtm:
      ...
      if (quoteType === "INDEX") {
        ...
        if (!yahooSortField) {
          throw new FinanceError({
            code: "bad_request",
            message:
              `Unsupported sortField: '${sortField}'. Supported fields: ${
                Object.keys(INDEX_SORT_MAP).join(", ")
              }`,
          });
        }
      } else if (quoteType === "EQUITY") {
      ...
      if (quoteType === "INDEX") {
        ...
        const pctChangeNode = buildRange(
          "percentchange",
          idxInput.percentChangeRange,
        );
        if (pctChangeNode) operands.push(pctChangeNode);

        const fiftyTwoWeekPctChangeNode = buildRange(
          "fiftytwowkpercentchange",
          idxInput.fiftyTwoWeekPercentChangeRange,
        );
        if (fiftyTwoWeekPctChangeNode) operands.push(fiftyTwoWeekPctChangeNode);

        const intradayPriceNode = buildRange(
          "intradayprice",
          idxInput.intradayPriceRange,
        );
        ...
        const intradayPriceChangeNode = buildRange(
          "intradaypricechange",
          idxInput.intradayPriceChangeRange,
        );
        ...
        const marketCapNode = buildRange(
          "intradaymarketcap",
          eqInput.marketCapRange,
        );
        if (marketCapNode) operands.push(marketCapNode);

        const peRatioNode = buildRange(
          "peratio.lasttwelvemonths",
          eqInput.peRatioRange,
        );
        ...
        const divYieldNode = buildRange(
          "dividendyield",
          eqInput.dividendYieldRange,
        );
        if (divYieldNode) operands.push(divYieldNode);

        const lastClosePriceBookValueNode = buildRange(
          "lastclosepricebookvalue.lasttwelvemonths",
          eqInput.lastClosePriceBookValueRange,
        );
        if (lastClosePriceBookValueNode) {
          operands.push(lastClosePriceBookValueNode);
        }

        // v2.4 Valuation AST builders:
        const lastCloseMarketCapNode = buildRange(
          "lastclosemarketcap.lasttwelvemonths",
          eqInput.lastCloseMarketCapRange,
        );
        if (lastCloseMarketCapNode) operands.push(lastCloseMarketCapNode);

        const lastCloseTevEbitdaNode = buildRange(
          "lastclosetevebitda.lasttwelvemonths",
          eqInput.lastCloseTevEbitdaRange,
        );
        if (lastCloseTevEbitdaNode) operands.push(lastCloseTevEbitdaNode);

        const lastClosePriceTangibleBookValueNode = buildRange(
          "lastclosepricetangiblebookvalue.lasttwelvemonths",
          eqInput.lastClosePriceTangibleBookValueRange,
        );
        if (lastClosePriceTangibleBookValueNode) {
          operands.push(lastClosePriceTangibleBookValueNode);
        }

        const lastCloseTevEbitNode = buildRange(
          "lastclosetevebit.lasttwelvemonths",
          eqInput.lastCloseTevEbitRange,
        );
        if (lastCloseTevEbitNode) operands.push(lastCloseTevEbitNode);

        const priceSalesRatioNode = buildRange(
          "lastclosemarketcaptotalrevenue.lasttwelvemonths",
          eqInput.priceSalesRatioRange,
        );
        if (priceSalesRatioNode) operands.push(priceSalesRatioNode);

        const lastCloseTevTotalRevenueNode = buildRange(
          "lastclosetevtotalrevenue.lasttwelvemonths",
          eqInput.lastCloseTevTotalRevenueRange,
        );
        if (lastCloseTevTotalRevenueNode) {
          operands.push(lastCloseTevTotalRevenueNode);
        }

        const lastClosePriceEarningsRatioNode = buildRange(
          "lastclosepriceearnings.lasttwelvemonths",
          eqInput.lastClosePriceEarningsRatioRange,
        );
        if (lastClosePriceEarningsRatioNode) {
          operands.push(lastClosePriceEarningsRatioNode);
        }

        const priceBookRatioQuarterlyNode = buildRange(
          "pricebookratio.quarterly",
          eqInput.priceBookRatioQuarterlyRange,
        );
        if (priceBookRatioQuarterlyNode) {
          operands.push(priceBookRatioQuarterlyNode);
        }

        const pegRatio5YrNode = buildRange(
          "pegratio_5y",
          eqInput.pegRatio5YrRange,
        );
        if (pegRatio5YrNode) operands.push(pegRatio5YrNode);

        const currentRatioNode = buildRange(
          "currentratio.lasttwelvemonths",
          eqInput.currentRatioRange,
        );
        if (currentRatioNode) operands.push(currentRatioNode);

        const grossProfitMarginNode = buildRange(
          "grossprofitmargin.lasttwelvemonths",
          eqInput.grossProfitMarginRange,
        );
        if (grossProfitMarginNode) operands.push(grossProfitMarginNode);

        const returnOnAssetsNode = buildRange(
          "returnonassets.lasttwelvemonths",
          eqInput.returnOnAssetsRange,
        );
        if (returnOnAssetsNode) operands.push(returnOnAssetsNode);

        const returnOnEquityNode = buildRange(
          "returnonequity.lasttwelvemonths",
          eqInput.returnOnEquityRange,
        );
        if (returnOnEquityNode) operands.push(returnOnEquityNode);

        const totalDebtEquityNode = buildRange(
          "totaldebtequity.lasttwelvemonths",
          eqInput.totalDebtEquityRange,
        );
        if (totalDebtEquityNode) operands.push(totalDebtEquityNode);

        const longTermDebtEquityNode = buildRange(
          "ltdebtequity.lasttwelvemonths",
          eqInput.longTermDebtEquityRange,
        );
        if (longTermDebtEquityNode) operands.push(longTermDebtEquityNode);

        const returnOnTotalCapitalNode = buildRange(
          "returnontotalcapital.lasttwelvemonths",
          eqInput.returnOnTotalCapitalRange,
        );
        if (returnOnTotalCapitalNode) operands.push(returnOnTotalCapitalNode);

        const netIncomeMarginNode = buildRange(
          "netincomemargin.lasttwelvemonths",
          eqInput.netIncomeMarginRange,
        );
        if (netIncomeMarginNode) operands.push(netIncomeMarginNode);

        const altmanZScoreNode = buildRange(
          "altmanzscoreusingtheaveragestockinformationforaperiod.lasttwelvemonths",
          eqInput.altmanZScoreRange,
        );
        if (altmanZScoreNode) operands.push(altmanZScoreNode);

        const quickRatioNode = buildRange(
          "quickratio.lasttwelvemonths",
          eqInput.quickRatioRange,
        );
        if (quickRatioNode) operands.push(quickRatioNode);

        const totalDebtEbitdaNode = buildRange(
          "totaldebtebitda.lasttwelvemonths",
          eqInput.totalDebtEbitdaRange,
        );
        if (totalDebtEbitdaNode) operands.push(totalDebtEbitdaNode);

        const ebitdaMarginNode = buildRange(
          "ebitdamargin.lasttwelvemonths",
          eqInput.ebitdaMarginRange,
        );
        if (ebitdaMarginNode) operands.push(ebitdaMarginNode);

        const netDebtEbitdaNode = buildRange(
          "netdebtebitda.lasttwelvemonths",
          eqInput.netDebtEbitdaRange,
        );
        if (netDebtEbitdaNode) operands.push(netDebtEbitdaNode);

        const epsGrowthNode = buildRange(
          "epsgrowth.lasttwelvemonths",
          eqInput.epsGrowthRange,
        );
        if (epsGrowthNode) operands.push(epsGrowthNode);

        const forwardDividendYieldNode = buildRange(
        ...
        const environmentalScoreNode = buildRange(
          "environmental_score",
          eqInput.environmentalScoreRange,
        );
        ...
        const governanceScoreNode = buildRange(
          "governance_score",
          eqInput.governanceScoreRange,
        );
        if (governanceScoreNode) operands.push(governanceScoreNode);

        const socialScoreNode = buildRange(
          "social_score",
          eqInput.socialScoreRange,
        );
        if (socialScoreNode) operands.push(socialScoreNode);

        const insiderOwnershipPercentNode = buildRange(
          "pctheldinsider",
          eqInput.insiderOwnershipPercentRange,
        );
        if (insiderOwnershipPercentNode) {
          operands.push(insiderOwnershipPercentNode);
        }

        const institutionalOwnershipPercentNode = buildRange(
          "pctheldinst",
          eqInput.institutionalOwnershipPercentRange,
        );
        if (institutionalOwnershipPercentNode) {
          operands.push(institutionalOwnershipPercentNode);
        }

        const highestControversyNode = buildCategorical(
          "highest_controversy",
          eqInput.highestControversyIn,
        );
        if (highestControversyNode) operands.push(highestControversyNode);
        const operatingCashFlowToCurrentLiabilitiesLtmNode = buildRange(
          "operatingcashflowtocurrentliabilities.lasttwelvemonths",
          eqInput.operatingCashFlowToCurrentLiabilitiesRange,
        );
        if (operatingCashFlowToCurrentLiabilitiesLtmNode) {
          operands.push(operatingCashFlowToCurrentLiabilitiesLtmNode);
        }
        const ebitdaInterestExpenseLtmNode = buildRange(
          "ebitdainterestexpense.lasttwelvemonths",
          eqInput.ebitdaInterestExpenseRange,
        );
        if (ebitdaInterestExpenseLtmNode) {
          operands.push(ebitdaInterestExpenseLtmNode);
        }
        const ebitInterestExpenseLtmNode = buildRange(
          "ebitinterestexpense.lasttwelvemonths",
          eqInput.ebitInterestExpenseRange,
        );
        if (ebitInterestExpenseLtmNode) {
          operands.push(ebitInterestExpenseLtmNode);
        }
        const totalRevenue1YrGrowthPercentLtmNode = buildRange(
          "totalrevenues1yrgrowth.lasttwelvemonths",
          eqInput.totalRevenue1YrGrowthRange,
        );
        if (totalRevenue1YrGrowthPercentLtmNode) {
          operands.push(totalRevenue1YrGrowthPercentLtmNode);
        }
        const netIncome1YrGrowthPercentLtmNode = buildRange(
          "netincome1yrgrowth.lasttwelvemonths",
          eqInput.netIncome1YrGrowthRange,
        );
        if (netIncome1YrGrowthPercentLtmNode) {
          operands.push(netIncome1YrGrowthPercentLtmNode);
        }
        const basicEpsContinuingOperationsLtmNode = buildRange(
          "basicepscontinuingoperations.lasttwelvemonths",
          eqInput.basicEpsContinuingOperationsRange,
        );
        if (basicEpsContinuingOperationsLtmNode) {
          operands.push(basicEpsContinuingOperationsLtmNode);
        }
        const revenueGrowthPercentYoYQuarterlyNode = buildRange(
          "quarterlyrevenuegrowth.quarterly",
          eqInput.revenueGrowthPercentYoYQuarterlyRange,
        );
        if (revenueGrowthPercentYoYQuarterlyNode) {
          operands.push(revenueGrowthPercentYoYQuarterlyNode);
        }
        const totalRevenueLtmNode = buildRange(
          "totalrevenues.lasttwelvemonths",
          eqInput.totalRevenueRange,
        );
        if (totalRevenueLtmNode) operands.push(totalRevenueLtmNode);
        const totalRevenueAnnualMarketCurrencyNode = buildRange(
          "total_revenue_market_currency.annual",
          eqInput.totalRevenueAnnualMarketCurrencyRange,
        );
        if (totalRevenueAnnualMarketCurrencyNode) {
          operands.push(totalRevenueAnnualMarketCurrencyNode);
        }
        const totalRevenuePerEmployeeAnnualMarketCurrencyNode = buildRange(
          "total_revenue_per_employee_annual_market_currency",
          eqInput.totalRevenuePerEmployeeAnnualMarketCurrencyRange,
        );
        if (totalRevenuePerEmployeeAnnualMarketCurrencyNode) {
          operands.push(totalRevenuePerEmployeeAnnualMarketCurrencyNode);
        }
        const netEpsBasicLtmNode = buildRange(
          "netepsbasic.lasttwelvemonths",
          eqInput.netEpsBasicRange,
        );
        if (netEpsBasicLtmNode) operands.push(netEpsBasicLtmNode);
        const ebitda1YrGrowthPercentLtmNode = buildRange(
          "ebitda1yrgrowth.lasttwelvemonths",
          eqInput.ebitda1YrGrowthRange,
        );
        if (ebitda1YrGrowthPercentLtmNode) {
          operands.push(ebitda1YrGrowthPercentLtmNode);
        }
        const dilutedEps1YrGrowthPercentLtmNode = buildRange(
          "dilutedeps1yrgrowth.lasttwelvemonths",
          eqInput.dilutedEps1YrGrowthRange,
        );
        if (dilutedEps1YrGrowthPercentLtmNode) {
          operands.push(dilutedEps1YrGrowthPercentLtmNode);
        }
        const netEpsDilutedLtmNode = buildRange(
          "netepsdiluted.lasttwelvemonths",
          eqInput.netEpsDilutedRange,
        );
        if (netEpsDilutedLtmNode) operands.push(netEpsDilutedLtmNode);
        const netIncomeIsLtmNode = buildRange(
          "netincomeis.lasttwelvemonths",
          eqInput.netIncomeIsRange,
        );
        if (netIncomeIsLtmNode) operands.push(netIncomeIsLtmNode);
        const netIncomeIsAnnualMarketCurrencyNode = buildRange(
          "netincomeismarketcurrency.annual",
          eqInput.netIncomeIsAnnualMarketCurrencyRange,
        );
        if (netIncomeIsAnnualMarketCurrencyNode) {
          operands.push(netIncomeIsAnnualMarketCurrencyNode);
        }
        const netIncomePerEmployeeAnnualMarketCurrencyNode = buildRange(
          "net_income_per_employee_annual_market_currency",
          eqInput.netIncomePerEmployeeAnnualMarketCurrencyRange,
        );
        if (netIncomePerEmployeeAnnualMarketCurrencyNode) {
          operands.push(netIncomePerEmployeeAnnualMarketCurrencyNode);
        }
        const operatingIncomeLtmNode = buildRange(
          "operatingincome.lasttwelvemonths",
          eqInput.operatingIncomeRange,
        );
        if (operatingIncomeLtmNode) operands.push(operatingIncomeLtmNode);
        const grossProfitLtmNode = buildRange(
          "grossprofit.lasttwelvemonths",
          eqInput.grossProfitRange,
        );
        if (grossProfitLtmNode) operands.push(grossProfitLtmNode);
        const ebitdaLtmNode = buildRange(
          "ebitda.lasttwelvemonths",
          eqInput.ebitdaRange,
        );
        if (ebitdaLtmNode) operands.push(ebitdaLtmNode);
        const dilutedEpsContinuingOperationsLtmNode = buildRange(
          "dilutedepscontinuingoperations.lasttwelvemonths",
          eqInput.dilutedEpsContinuingOperationsRange,
        );
        if (dilutedEpsContinuingOperationsLtmNode) {
          operands.push(dilutedEpsContinuingOperationsLtmNode);
        }
        const ebitLtmNode = buildRange(
          "ebit.lasttwelvemonths",
          eqInput.ebitRange,
        );
        if (ebitLtmNode) operands.push(ebitLtmNode);

        const bookValueShareNode = buildRange(
          "bookvalueshare.lasttwelvemonths",
          eqInput.bookValueShareRange,
        );
        if (bookValueShareNode) operands.push(bookValueShareNode);
        const totalSharesOutstandingNode = buildRange(
          "totalsharesoutstanding",
          eqInput.totalSharesOutstandingRange,
        );
        if (totalSharesOutstandingNode) {
          operands.push(totalSharesOutstandingNode);
        }
        const totalDebtNode = buildRange(
          "totaldebt.lasttwelvemonths",
          eqInput.totalDebtRange,
        );
        if (totalDebtNode) operands.push(totalDebtNode);
        const totalAssetsNode = buildRange(
          "totalassets.lasttwelvemonths",
          eqInput.totalAssetsRange,
        );
        if (totalAssetsNode) operands.push(totalAssetsNode);
        const totalCashAndShortTermInvestmentsNode = buildRange(
          "totalcashandshortterminvestments.lasttwelvemonths",
          eqInput.totalCashAndShortTermInvestmentsRange,
        );
        if (totalCashAndShortTermInvestmentsNode) {
          operands.push(totalCashAndShortTermInvestmentsNode);
        }
        const totalCurrentAssetsNode = buildRange(
          "totalcurrentassets.lasttwelvemonths",
          eqInput.totalCurrentAssetsRange,
        );
        if (totalCurrentAssetsNode) operands.push(totalCurrentAssetsNode);
        const totalEquityNode = buildRange(
          "totalequity.lasttwelvemonths",
          eqInput.totalEquityRange,
        );
        if (totalEquityNode) operands.push(totalEquityNode);
        const totalCommonSharesOutstandingNode = buildRange(
          "totalcommonsharesoutstanding.lasttwelvemonths",
          eqInput.totalCommonSharesOutstandingRange,
        );
        if (totalCommonSharesOutstandingNode) {
          operands.push(totalCommonSharesOutstandingNode);
        }
        const totalCurrentLiabilitiesNode = buildRange(
          "totalcurrentliabilities.lasttwelvemonths",
          eqInput.totalCurrentLiabilitiesRange,
        );
        if (totalCurrentLiabilitiesNode) {
          operands.push(totalCurrentLiabilitiesNode);
        }
        const totalCommonEquityNode = buildRange(
          "totalcommonequity.lasttwelvemonths",
          eqInput.totalCommonEquityRange,
        );
        if (totalCommonEquityNode) operands.push(totalCommonEquityNode);
        const fullTimeEmployeesNode = buildRange(
          "fulltimeemployees.annual",
          eqInput.fullTimeEmployeesRange,
        );
        if (fullTimeEmployeesNode) operands.push(fullTimeEmployeesNode);
        const cashOnHandQuarterlyMarketCurrencyNode = buildRange(
          "cash_on_hand_quarterly_market_currency",
          eqInput.cashOnHandQuarterlyMarketCurrencyRange,
        );
        if (cashOnHandQuarterlyMarketCurrencyNode) {
          operands.push(cashOnHandQuarterlyMarketCurrencyNode);
        }
        const cashFromOperations1YrGrowthPercentNode = buildRange(
          "cashfromoperations1yrgrowth.lasttwelvemonths",
          eqInput.cashFromOperations1YrGrowthPercentRange,
        );
        if (cashFromOperations1YrGrowthPercentNode) {
          operands.push(cashFromOperations1YrGrowthPercentNode);
        }
        const unleveredFreeCashFlowNode = buildRange(
          "unleveredfreecashflow.lasttwelvemonths",
          eqInput.unleveredFreeCashFlowRange,
        );
        if (unleveredFreeCashFlowNode) operands.push(unleveredFreeCashFlowNode);
        const leveredFreeCashFlowNode = buildRange(
          "leveredfreecashflow.lasttwelvemonths",
          eqInput.leveredFreeCashFlowRange,
        );
        if (leveredFreeCashFlowNode) operands.push(leveredFreeCashFlowNode);
        const leveredFreeCashFlow1YrGrowthPercentNode = buildRange(
          "leveredfreecashflow1yrgrowth.lasttwelvemonths",
          eqInput.leveredFreeCashFlow1YrGrowthPercentRange,
        );
        if (leveredFreeCashFlow1YrGrowthPercentNode) {
          operands.push(leveredFreeCashFlow1YrGrowthPercentNode);
        }
        const dividendPerShareNode = buildRange(
          "dividendpershare.lasttwelvemonths",
          eqInput.dividendPerShareRange,
        );
        if (dividendPerShareNode) operands.push(dividendPerShareNode);
        const capitalExpenditureNode = buildRange(
          "capitalexpenditure.lasttwelvemonths",
          eqInput.capitalExpenditureRange,
        );
        if (capitalExpenditureNode) operands.push(capitalExpenditureNode);
        const forwardDividendPerShareNode = buildRange(
          "forward_dividend_per_share",
          eqInput.forwardDividendPerShareRange,
        );
        if (forwardDividendPerShareNode) {
          operands.push(forwardDividendPerShareNode);
        }
        const cashFromOperationsNode = buildRange(
      ...
      if (operands.length > 20) {
        throw new FinanceError({
          code: "bad_request",
          message:
            `Query complexity limit exceeded: maximum of 20 top-level filter groups allowed. Got ${operands.length}.`,
        });
      }
        ...
        for (const field of requestedFields) {
          switch (field) {
            ...
            case "regularMarketPrice":
              mapped.regularMarketPrice = lookup([
                "regularMarketPrice",
                "intradayprice",
                "price",
              ]);
            ...
            case "regularMarketChange":
              mapped.regularMarketChange = lookup([
                "regularMarketChange",
                "intradaypricechange",
                "change",
              ]);
            ...
            case "regularMarketChangePercent":
              mapped.regularMarketChangePercent = lookup([
                "regularMarketChangePercent",
                "percentchange",
                "changePercent",
              ]);
            ...
            case "regularMarketVolume":
              mapped.regularMarketVolume = lookup([
                "regularMarketVolume",
                "dayvolume",
                "volume",
              ]);
            ...
            case "averageDailyVolume3Month":
              mapped.averageDailyVolume3Month = lookup([
                "avgDailyVol3m",
                "avgdailyvol3m",
            ...
            case "fiftyTwoWeekPercentChange":
              mapped.fiftyTwoWeekPercentChange = lookup([
                "fiftyTwoWeekChangePercent",
                "fiftytwowkpercentchange",
              ]);
              break;
            case "marketCap":
              mapped.marketCap = lookup([
                "marketCap",
                "intradaymarketcap",
                "marketcap",
              ]);
            ...
            case "lastClosePriceBookValueLtm":
              mapped.lastClosePriceBookValueLtm = lookup([
                "lastClosePriceBookValueLtm",
                "lastclosepricebookvalueltm",
              ]);
              break;
            // v2.4 Valuation response mappings:
            case "lastCloseMarketCapLtm":
              mapped.lastCloseMarketCapLtm = lookup([
                "lastCloseMarketCapLtm",
                "lastclosemarketcapltm",
              ]);
              break;
            case "lastCloseTevEbitdaLtm":
              mapped.lastCloseTevEbitdaLtm = lookup([
                "lastCloseTevEbitdaLtm",
                "lastclosetevebitdaltm",
              ]);
              break;
            case "lastClosePriceTangibleBookValueLtm":
              mapped.lastClosePriceTangibleBookValueLtm = lookup([
                "lastClosePriceTangibleBookValueLtm",
                "lastclosepricetangiblebookvalueltm",
              ]);
              break;
            case "lastCloseTevEbitLtm":
              mapped.lastCloseTevEbitLtm = lookup([
                "lastCloseTevEbitLtm",
                "lastclosetevebitltm",
              ]);
              break;
            case "priceSalesRatioLtm":
              mapped.priceSalesRatioLtm = lookup([
                "priceSalesRatioLtm",
                "pricesalesratioltm",
              ]);
              break;
            case "lastCloseTevTotalRevenueLtm":
              mapped.lastCloseTevTotalRevenueLtm = lookup([
                "lastCloseTevTotalRevenueLtm",
                "lastclosetevtotalrevenueltm",
              ]);
              break;
            case "lastClosePriceEarningsRatioLtm":
              mapped.lastClosePriceEarningsRatioLtm = lookup([
                "lastClosePriceEarningsRatioLtm",
                "lastclosepriceearningsratioltm",
              ]);
              break;
            case "priceBookRatioQuarterly":
              mapped.priceBookRatioQuarterly = lookup([
                "priceBookRatioQuarterly",
                "pricebookratioquarterly",
              ]);
              break;
            // v2.4 ESG & Ownership response mappings:
            case "environmentalScore":
              mapped.environmentalScore = lookup([
                "environmentalScore",
                "environmentalscore",
              ]);
              break;
            ...
            case "highestControversy":
              mapped.highestControversy = lookup([
                "highestControversy",
                "highestcontroversy",
              ]);
              break;
            case "insiderOwnershipPercent":
              mapped.insiderOwnershipPercent = lookup([
                "insiderOwnershipPercent",
                "insiderownershippercent",
                "percentOfSharesOutHeldByInsider",
              ]);
              break;
            case "institutionalOwnershipPercent":
              mapped.institutionalOwnershipPercent = lookup([
                "institutionalOwnershipPercent",
                "institutionalownershippercent",
                "percentOfSharesOutHeldByInstitutions",
              ]);
              break;
            case "pegRatio5Yr":
            ...
            case "currentRatioLtm":
              mapped.currentRatioLtm = lookup([
                "currentRatioLtm",
                "currentratioltm",
              ]);
              break;
            case "grossProfitMarginPercentLtm":
              mapped.grossProfitMarginPercentLtm = lookup([
                "grossProfitMarginPercentLtm",
                "grossprofitmarginpercentltm",
              ]);
              break;
            case "returnOnAssetsPercentLtm":
              mapped.returnOnAssetsPercentLtm = lookup([
                "returnOnAssetsPercentLtm",
                "returnonassetspercentltm",
              ]);
              break;
            case "returnOnEquityPercentLtm":
              mapped.returnOnEquityPercentLtm = lookup([
                "returnOnEquityPercentLtm",
                "returnonequitypercentltm",
              ]);
              break;
            case "totalDebtEquityPercentLtm":
              mapped.totalDebtEquityPercentLtm = lookup([
                "totalDebtEquityPercentLtm",
                "totaldebtequitypercentltm",
              ]);
              break;
            case "longTermDebtEquityPercentLtm":
              mapped.longTermDebtEquityPercentLtm = lookup([
                "longTermDebtEquityPercentLtm",
                "longtermdebtequitypercentltm",
              ]);
              break;
            case "returnOnTotalCapitalLtm":
              mapped.returnOnTotalCapitalLtm = lookup([
                "returnOnTotalCapitalLtm",
                "returnontotalcapitalltm",
              ]);
              break;
            case "netIncomeMarginPercentLtm":
              mapped.netIncomeMarginPercentLtm = lookup([
                "netIncomeMarginPercentLtm",
                "netincomemarginpercentltm",
              ]);
              break;
            case "altmanZScoreLtm":
              mapped.altmanZScoreLtm = lookup([
                "altmanZScoreLtm",
                "altmanzscoreltm",
              ]);
              break;
            ...
            case "totalDebtEbitdaLtm":
              mapped.totalDebtEbitdaLtm = lookup([
                "totalDebtEbitdaLtm",
                "totaldebtebitdaltm",
              ]);
              break;
            case "ebitdaMarginPercentLtm":
              mapped.ebitdaMarginPercentLtm = lookup([
                "ebitdaMarginPercentLtm",
                "ebitdamarginpercentltm",
              ]);
              break;
            case "netDebtEbitdaLtm":
              mapped.netDebtEbitdaLtm = lookup([
                "netDebtEbitdaLtm",
                "netdebtebitdaltm",
              ]);
              break;
            case "epsGrowthPercentLtm":
              mapped.epsGrowthPercentLtm = lookup([
                "epsGrowthPercentLtm",
                "epsgrowthpercentltm",
              ]);
              break;
            case "forwardDividendYieldPercent":
              mapped.forwardDividendYieldPercent = lookup([
                "forwardDividendYieldPercent",
                "forwarddividendyieldpercent",
              ]);
              break;
            case "operatingCashFlowToCurrentLiabilitiesLtm":
              mapped.operatingCashFlowToCurrentLiabilitiesLtm = lookup([
                "operatingCashFlowToCurrentLiabilitiesLtm",
                "operatingcashflowtocurrentliabilitiesltm",
              ]);
              break;
            case "ebitdaInterestExpenseLtm":
              mapped.ebitdaInterestExpenseLtm = lookup([
                "ebitdaInterestExpenseLtm",
                "ebitdainterestexpenseltm",
              ]);
              break;
            case "ebitInterestExpenseLtm":
              mapped.ebitInterestExpenseLtm = lookup([
                "ebitInterestExpenseLtm",
                "ebitinterestexpenseltm",
              ]);
              break;
            case "totalRevenue1YrGrowthPercentLtm":
              mapped.totalRevenue1YrGrowthPercentLtm = lookup([
                "totalRevenue1YrGrowthPercentLtm",
                "totalrevenue1yrgrowthpercentltm",
              ]);
              break;
            case "netIncome1YrGrowthPercentLtm":
              mapped.netIncome1YrGrowthPercentLtm = lookup([
                "netIncome1YrGrowthPercentLtm",
                "netincome1yrgrowthpercentltm",
              ]);
              break;
            case "basicEpsContinuingOperationsLtm":
              mapped.basicEpsContinuingOperationsLtm = lookup([
                "basicEpsContinuingOperationsLtm",
                "basicepscontinuingoperationsltm",
              ]);
              break;
            case "revenueGrowthPercentYoYQuarterly":
              mapped.revenueGrowthPercentYoYQuarterly = lookup([
                "revenueGrowthPercentYoYQuarterly",
                "revenuegrowthpercentyoyquarterly",
              ]);
              break;
            case "totalRevenueLtm":
              mapped.totalRevenueLtm = lookup([
                "totalRevenueLtm",
                "totalrevenueltm",
              ]);
              break;
            case "totalRevenueAnnualMarketCurrency":
              mapped.totalRevenueAnnualMarketCurrency = lookup([
                "totalRevenueAnnualMarketCurrency",
                "totalrevenueannualmarketcurrency",
              ]);
              break;
            case "totalRevenuePerEmployeeAnnualMarketCurrency":
              mapped.totalRevenuePerEmployeeAnnualMarketCurrency = lookup([
                "totalRevenuePerEmployeeAnnualMarketCurrency",
                "totalrevenueperemployeeannualmarketcurrency",
              ]);
              break;
            case "netEpsBasicLtm":
              mapped.netEpsBasicLtm = lookup([
                "netEpsBasicLtm",
                "netepsbasicltm",
              ]);
              break;
            case "ebitda1YrGrowthPercentLtm":
              mapped.ebitda1YrGrowthPercentLtm = lookup([
                "ebitda1YrGrowthPercentLtm",
                "ebitda1yrgrowthpercentltm",
              ]);
              break;
            case "dilutedEps1YrGrowthPercentLtm":
              mapped.dilutedEps1YrGrowthPercentLtm = lookup([
                "dilutedEps1YrGrowthPercentLtm",
                "dilutedeps1yrgrowthpercentltm",
              ]);
              break;
            case "netEpsDilutedLtm":
              mapped.netEpsDilutedLtm = lookup([
                "netEpsDilutedLtm",
                "netepsdilutedltm",
              ]);
              break;
            case "netIncomeIsLtm":
              mapped.netIncomeIsLtm = lookup([
                "netIncomeIsLtm",
                "netincomeisltm",
              ]);
              break;
            case "netIncomeIsAnnualMarketCurrency":
              mapped.netIncomeIsAnnualMarketCurrency = lookup([
                "netIncomeIsAnnualMarketCurrency",
                "netincomeisannualmarketcurrency",
              ]);
              break;
            case "netIncomePerEmployeeAnnualMarketCurrency":
              mapped.netIncomePerEmployeeAnnualMarketCurrency = lookup([
                "netIncomePerEmployeeAnnualMarketCurrency",
                "netincomeperemployeeannualmarketcurrency",
              ]);
              break;
            case "operatingIncomeLtm":
              mapped.operatingIncomeLtm = lookup([
                "operatingIncomeLtm",
                "operatingincomeltm",
              ]);
              break;
            case "grossProfitLtm":
              mapped.grossProfitLtm = lookup([
                "grossProfitLtm",
                "grossprofitltm",
              ]);
              break;
            ...
            case "dilutedEpsContinuingOperationsLtm":
              mapped.dilutedEpsContinuingOperationsLtm = lookup([
                "dilutedEpsContinuingOperationsLtm",
                "dilutedepscontinuingoperationsltm",
              ]);
              break;
            ...
            case "bookValueShareLtm":
              mapped.bookValueShareLtm = lookup([
                "bookValueShareLtm",
                "bookvalueshareltm",
              ]);
              break;
            case "totalSharesOutstanding":
              mapped.totalSharesOutstanding = lookup([
                "totalSharesOutstanding",
                "totalsharesoutstanding",
              ]);
              break;
            case "totalDebtLtm":
              ...
              break;
            case "totalAssetsLtm":
              mapped.totalAssetsLtm = lookup([
                "totalAssetsLtm",
                "totalassetsltm",
              ]);
              break;
            case "totalCashAndShortTermInvestmentsLtm":
              mapped.totalCashAndShortTermInvestmentsLtm = lookup([
                "totalCashAndShortTermInvestmentsLtm",
                "totalcashandshortterminvestmentsltm",
              ]);
              break;
            case "totalCurrentAssetsLtm":
              mapped.totalCurrentAssetsLtm = lookup([
                "totalCurrentAssetsLtm",
                "totalcurrentassetsltm",
              ]);
              break;
            case "totalEquityLtm":
              mapped.totalEquityLtm = lookup([
                "totalEquityLtm",
                "totalequityltm",
              ]);
              break;
            case "totalCommonSharesOutstandingLtm":
              mapped.totalCommonSharesOutstandingLtm = lookup([
                "totalCommonSharesOutstandingLtm",
                "totalcommonsharesoutstandingltm",
              ]);
              break;
            case "totalCurrentLiabilitiesLtm":
              mapped.totalCurrentLiabilitiesLtm = lookup([
                "totalCurrentLiabilitiesLtm",
                "totalcurrentliabilitiesltm",
              ]);
              break;
            case "totalCommonEquityLtm":
              mapped.totalCommonEquityLtm = lookup([
                "totalCommonEquityLtm",
                "totalcommonequityltm",
              ]);
              break;
            case "fullTimeEmployees":
              mapped.fullTimeEmployees = lookup([
                "fullTimeEmployees",
                "fulltimeemployees",
              ]);
              break;
            case "cashOnHandQuarterlyMarketCurrency":
              mapped.cashOnHandQuarterlyMarketCurrency = lookup([
                "cashOnHandQuarterlyMarketCurrency",
                "cashonhandquarterlymarketcurrency",
              ]);
              break;
            case "cashFromOperations1YrGrowthPercentLtm":
              mapped.cashFromOperations1YrGrowthPercentLtm = lookup([
                "cashFromOperations1YrGrowthPercentLtm",
                "cashfromoperations1yrgrowthpercentltm",
              ]);
              break;
            case "unleveredFreeCashFlowLtm":
              mapped.unleveredFreeCashFlowLtm = lookup([
                "unleveredFreeCashFlowLtm",
                "unleveredfreecashflowltm",
              ]);
              break;
            case "leveredFreeCashFlowLtm":
              mapped.leveredFreeCashFlowLtm = lookup([
                "leveredFreeCashFlowLtm",
                "leveredfreecashflowltm",
              ]);
              break;
            case "leveredFreeCashFlow1YrGrowthPercentLtm":
              mapped.leveredFreeCashFlow1YrGrowthPercentLtm = lookup([
                "leveredFreeCashFlow1YrGrowthPercentLtm",
                "leveredfreecashflow1yrgrowthpercentltm",
              ]);
              break;
            case "dividendPerShareLtm":
              mapped.dividendPerShareLtm = lookup([
                "dividendPerShareLtm",
                "dividendpershareltm",
              ]);
              break;
            case "capitalExpenditureLtm":
              mapped.capitalExpenditureLtm = lookup([
                "capitalExpenditureLtm",
                "capitalexpenditureltm",
              ]);
              break;
            case "forwardDividendPerShare":
              mapped.forwardDividendPerShare = lookup([
                "forwardDividendPerShare",
                "forwarddividendpershare",
              ]);
              break;
            case "cashFromOperationsLtm":
              mapped.cashFromOperationsLtm = lookup([
                "cashFromOperationsLtm",
                "cashfromoperationsltm",
              ]);
              break;
            case "sector":
      ...
      const nextOffset = offset + records.length < totalCount

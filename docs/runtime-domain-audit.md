...
| Thread-specific operation catalog methods                                 | [Core operation queries](../plugins/core-http/adapters/http/core/operations/index.ts), consumed by the server after thread authorization. Core supplies metadata criteria to the generic catalog API; runtime owns SQL and index selection without exposing its session or table names. |
| Public/participant/tool/internal visibility union                         | [Core event policy](../plugins/core/shared/events/index.ts); runtime stores opaque metadata. Message history projection remains participant-aware in Core.                                                                                                                              |
...
| Tool, reasoning and conversation content role suggestions                 | Owning plugin primitives supply role literals; runtime accepts an opaque role string.                                                                                                                                                                                                   |
| Conversational ephemeral event type suggestions                           | Owning plugin producers supply event names; runtime accepts an arbitrary non-empty type.                                                                                                                                                                                                |

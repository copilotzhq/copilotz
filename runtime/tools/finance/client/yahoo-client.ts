...
export async function acquireCookieAndCrumb(
  ...
  try {
    ...
    if (!cookie) {
      throw new FinanceError({
        ...
        message:
          "Failed to acquire cookie from fc.yahoo.com (no set-cookie header)",
      });
    }
    ...
    if (!crumbResponse.ok) {
      ...
      throw new FinanceError({
        code: "upstream_unavailable",
        message:
          `Failed to acquire crumb from Yahoo: HTTP ${crumbResponse.status}`,
      });
    }
    ...
    throw new FinanceError({
      code: "upstream_unavailable",
      message: `Network error during cookie/crumb acquisition: ${
  ...
  try {
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        throw new FinanceError({
          code: "upstream_unavailable",
          message: `Network error during Yahoo request: ${
            err instanceof Error ? err.message : String(err)
          }`,
          cause: err,
        });

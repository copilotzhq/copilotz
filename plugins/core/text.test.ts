...
async function createFixture(
  ...
  const policyAgent = Object.freeze({ ...agent, ...workflowOptions });
...
async function startContentRun(
  ...
  await boundCollection(fixture.engine, "thread").create({
    id: "thread-a",
    participantIds: ["user-a", agentParticipantId],
...
Deno.test("participant-relative history labels peer agents and enforces tool and reasoning visibility", async () => {
  ...
  try {
    ...
    await boundCollection(fixture.engine, "message").create({
      ...
      namespace: "tenant-a",
      threadId: "thread-a",
      routing: { senderId: "agent-south", recipientIds: [] },
    ...
    const root = await boundCollection(fixture.engine, "message").create({
      id: "message:user",
      threadId: "thread-a",
      senderId: "user-a",
...
Deno.test("A55 relocated workflow owners remain factory-first and runtime-neutral", async () => {
  const owners = new URL("../../runtime/", import.meta.url);
  for (
    const module of [
      "agents/index.ts",
      "agents/config.ts",
      ...
      "events/workflow-id.ts",
      "events/workflow-metadata.ts",
      "tools/catalog.ts",
      "tools/executor.ts",
      "tools/jq-pipeline.ts",
      "tools/types.ts",

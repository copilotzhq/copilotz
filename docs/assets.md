...
You can keep a global asset store while letting specific agents opt out of persisting assets they generate:
...
- assets generated in that agent's own message content are not written to the shared asset store
- assets generated by tools that agent invoked are sanitized instead of being stored as `asset://` refs
- `ASSET_CREATED` is not emitted for those generated blobs

This setting does not affect user-uploaded assets, existing `asset://` refs, or plain external URLs.

...
Copilotz ships with adapters for OpenAI, Anthropic, Gemini, Groq, DeepSeek, Ollama, and MiniMax. You can add your own by implementing the `ProviderFactory` interface.
...
from provider-specific environment variables (for example
`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) and fall back to `LLM_API_KEY`.
...
A provider is a factory function that takes a `ProviderConfig` and returns a `ProviderAPI` object:

```typescript
import type { ProviderFactory, ProviderConfig, ChatMessage, ExtractedPart, ProviderUsageUpdate } from "@copilotz/copilotz";
...
- `LLMRuntimeConfig` / `ProviderConfig`: runtime config actually passed to the provider
...
Many providers (Together, Fireworks, Perplexity, etc.) expose an OpenAI-compatible API. Use the built-in `openai` provider with a custom `baseUrl`:

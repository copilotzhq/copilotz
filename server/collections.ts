...
import type { CollectionsManager, Copilotz } from "@/index.ts";
import type {
...
export function createCollectionHandlers(
  ...
  const definitions = new Map<string, CollectionDefinition>(
    (copilotz.config.collections ?? []).map((
      definition,
  ...
  const getCollectionCrud = (
    ...
    const effectiveNamespace = namespace || (copilotz as any).config?.namespace;
    const scopedManager = effectiveNamespace
      ? manager.withNamespace(effectiveNamespace)
      : manager;
    const coll = scopedManager[collectionName];
  ...
  return {
    listCollections: (): string[] => {
      if (!manager || typeof manager.getCollectionNames !== "function") {
        return [];
      }
    ...
    search: (
      ...
      if (!("search" in coll)) {
        throw new Error(
          `Collection '${collectionName}' does not support search`,
        );
      }
      const crud = coll as {
        search: (

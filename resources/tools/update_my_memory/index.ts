...
export default {
  ...
  description:
    "Store important learnings or preferences that you want to remember across conversations. Use this to persist insights about users, topics you've learned, or any information that would be useful to recall in future conversations.",
  inputSchema: {
    type: "object",
    properties: {
      key: {
        type: "string",
        description:
          "What to remember (e.g., 'user_preference', 'learned_fact', 'expertise', 'working_memory')",
      },
      value: {
        type: "string",
  ...
  execute: async (args: UpdateMyMemoryParams, context?: ToolContext) => {
    ...
    try {
      const agentParticipant = await participantCollection.resolveByExternalId(
        agentId,
      );
      ...
      if (!agentParticipant) {
        return {
          success: false,
          error:
            `Agent node not found for "${agentId}". Memory update skipped.`,
        };
      }

      const metadata = {
        ...((agentParticipant.metadata ?? {}) as Record<string, unknown>),
      };
      ...
      await participantCollection.upsertIdentity({
        externalId: agentId,
        participantType: (agentParticipant.participantType ?? "agent") as
          | "human"
          | "agent",

...
export const readFileAction = defineAction({
  ...
  execute: async ({
    ...
    try {
          ...
          try {
            ...
            if (stat.size > MAX_FILE_SIZE_BYTES) {
              throw new Error(
                ...
                }KB — too large to read without a range. ` +
      ...
      const autoTruncated = noRangeSpecified &&
        result.totalLines > AUTO_TRUNCATE_LINES;
      ...
      return {
        ...
        ...(autoTruncated && {
          autoTruncated: true,
          hint:
            `File has ${result.totalLines} lines. Showing lines 1–${AUTO_TRUNCATE_LINES}. Use startLine/endLine to read a specific range.`,
        }),

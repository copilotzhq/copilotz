# Runtime

This section explains the user-relevant runtime foundations that sit underneath
Copilotz resources and application endpoints.

## Runtime Topics

- [How Resource Loading Works](./how-resource-loading-works.md)
- [How Threads Work](./how-threads-work.md)
- [How Events Work](./how-events-work.md)
- [Goals](./goals.md)
- [How the Graph Works](./how-the-graph-works.md)
- [Namespaces and Multi-Tenancy](./namespaces-and-multi-tenancy.md)
- [App Dispatcher and Endpoints](./app-dispatcher-and-endpoints.md)
- [Assets and Media](./assets-and-media.md)

## Use This Section For

- understanding how Copilotz consumes resources
- choosing the right persistence layer
- reasoning about execution flow
- mapping runtime behavior to transport endpoints

## Related Pages

- [Start Here](../start-here/README.md)
- [Resources](../resources/README.md)
- [Reference](../reference/README.md)

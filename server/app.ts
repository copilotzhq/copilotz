...
export interface WithAppOptions {
  ...
  /**
   * Log every runtime event generated by app channel deliveries.
   *
   * Set to `true` to write compact event records to `console.log`, or provide a
   * function to capture structured events in application logs.
   */
  ...
  try {
    ...
    if (logger === true) {
      const eventType = typeof event === "object" && event !== null &&
        ...
        : "unknown";
      console.log("[copilotz:app:event]", {
        type: eventType,
